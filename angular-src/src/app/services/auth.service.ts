...
export class AuthService {
  ...
  user: any;
  code: string;
  constructor(private http:Http,private shareService: SharedServiceService, private location: Location,private router:Router) {
    ...
    if (this.code) {
      ...
      this
        .loginProvider(this.code, this.tempConfigProvider.authEndpoint)
        .subscribe(data => {
          this
          ...
          this
            .router
            .navigate(['']);
         // this.setSmallProfileImage();
        })
    } 
    else {
      if(localStorage.getItem('id_token')){
        this.shareService.observeTokenSubject('token');
      }
    }
  }
  ...
  getUser(id: string){
    ...
    headers.append('Authorization', this.authToken);
    headers.append('Content-Type','application/json');
    return this.http.get('http://localhost:3000/users/'+id,{headers: headers})
      .map(res => res.json());
  }
  getAllUser(){
    let headers = new Headers();
