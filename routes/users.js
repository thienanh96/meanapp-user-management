...
router.put('/profile', passport.authenticate('jwt', { session: false }), (req, res, next) => {
    if ((req.body.role != null || undefined) || (req.body.username != null || undefined))
        return res.json({ success: false, msg: 'Cannot edit your role or username!' });
        ...
        .waterfall([
            function(done) {
                User.getUser({
                    ...
                    if (err || !user)
                        return res.json({ success: false, msg: 'Errors have occurred!' });
                    if (user) {
                        done(err, user)
                    }
                })
                ...
                User.comparePassword(req.body.old_password, user.password, (err, isMatch) => {
                    if (isMatch) {
                        if (req.body.new_password == req.body.re_password)
                            ...
                            res.json({ success: false, msg: 'Password does not match the confirm password !' })
                    }
                    if (!isMatch) {
        ...
        async.waterfall([
            ...
            function(user, done) {
                User.getUser({
                    ...
                    if (result)
                        return res.json({ success: false, msg: 'Exist email , try another one !' });
                    else {
                        var newUser = {
    ...
    if (req.body.editType === 'DELETE_NOTIFICATION') {
        let deleteCount = req.body.deleteCount;
        if (deleteCount === 'all') deleteCount = req.user.notification.length;
        console.log('asss: ' + deleteCount)
        let query = {
            $pull: {
                notification: {
                    id: {
                        $gte: req.user.notification.length - deleteCount
                    }
                }
            }
        };
...
router.put('/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    if (req.body.password != undefined || null || '')
        res.json({ success: false, msg: 'Cannot change password !' })
    async.waterfall([
        function(done) {
            let correctUser = {
            ...
            auth.updatePermission(correctUser, (user) => {
                ...
                if (auth.containAppliedTo && auth.containAction) {
                    done(null, user);
                } else {
...
router.put('/profile/avatar', passport.authenticate('jwt', { session: false }), (req, res) => {
    if (req.body.editType == 'EDIT_CURRENT_AVATAR') {
        ...
        if (req.body.displayImageType == 'social') {
            query = {
                'avatar.displayImageType': req.body.displayImageType
            }
        }
        if (req.body.displayImageType == 'local') {
            query = {
                'avatar.displayImageType': req.body.displayImageType,
                'avatar.local.displayImageID': req.body.displayImageID
            };
        }
        User
            .correctUser({
...
function convertImageListToBase64(imageList) {
    let arr_list = [];
    for (let i = 0; i < imageList.length; i++) {
        let path_local = imageList[i].path_local;
        let typeImage = imageList[i].typeImage;
...
router.get('/profile', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    arr_img = convertImageListToBase64(arr_img);
        ...
        if (err) {
            ...
            if (!user) {
                ...
                let profileUser = {
                    ...
                    avatar: {
                        path_social: user.avatar.path_social,
                        local: {
                            image: convertImageListToBase64(user.avatar.local.image),
                            displayImageID: user.avatar.local.displayImageID
                        },
                        displayImageType: user.avatar.displayImageType
                    },
...
router.delete('/:id', passport.authenticate('jwt', { session: false }), (req, res, next) => {
    ...
    if (auth.containAction && auth.containAppliedTo)
        return next();
    else
        return res.json({ success: false, msg: 'Unauthorized !' });
}, (req, res) => {
...
router.post('/upload/image', passport.authenticate('jwt', { session: false }), function(req, res, next) {
    ...
    upload(req, res, function(err) {
        ...
        if (req.files[0] != undefined || null) {
            ...
            User.correctUser({
                _id: req.user._id
            }, editAvatarInfo, (err, user) => {
                let length = user.avatar.local.image.length;
                User.correctUser({
                    _id: req.user._id
                }, {
                    'avatar.local.displayImageID': user.avatar.local.image[length - 1]._id
                }, (err, re) => {
                    var base64Code = new Buffer(fs.readFileSync(req.files[0].path)).toString('base64');
                    let prefix = 'data:' + req.files[0].mimetype + ';base64,';
                    res.json({
                        success: true,
                        base64ImageData: prefix + base64Code,
                        imageID: re.avatar.local.displayImageID
                    });
                })
                if (err)
                    throw err;
